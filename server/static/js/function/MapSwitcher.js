...
export default class MapSwitcher {
    constructor() {
        ...
        Logger.debug('MapSwitcher', '当前地图类型:', this.currentMapType);
        // 确保脚本加载完成后再初始化
        this.initializeMap();
    }
    ...
    async initializeMap() {
        try {
            ...
            Logger.info('MapSwitcher', '初始化完成');
        } catch (error) {
            Logger.error('MapSwitcher', '地图初始化失败:', error);
        ...
        return new Promise(resolve => {
            const checkDOM = () => {
                ...
                if (mapControls && switchMapBtn && switchDisplayBtn) {
                    Logger.debug('MapSwitcher', 'DOM元素已加载');
                    resolve();
                } else {
                    Logger.debug('MapSwitcher', '等待DOM元素加载...');
    ...
    initSwitchButton() {
        Logger.debug('MapSwitcher', '初始化切换按钮');
        const switchMapBtn = document.getElementById('switchMapType');
        const switchDisplayBtn = document.getElementById('switchDisplayMode');

        if (!switchMapBtn || !switchDisplayBtn) {
            Logger.error('MapSwitcher', '找不到切换按钮:', {
        ...
        switchMapBtn.addEventListener('click', async (e) => {
            Logger.debug('MapSwitcher', '切换按钮被点击');
            e.preventDefault(); // 防止事件冒泡
            
            // 切换地图类型
            this.currentMapType = this.currentMapType === 'AMAP' ? 'ECHARTS' : 'AMAP';
            Logger.debug('MapSwitcher', '新地图类型:', this.currentMapType);
    ...
    updateSwitchButtonText(element) {
        ...
        Logger.debug('MapSwitcher', '更新按钮文本为:', `切换至${text}地图`);
    ...
    async initMap() {
        ...
        try {
            ...
            Logger.error('MapSwitcher', '初始化地图失败:', error);
    ...
    setWebSocketManager(wsManager) {
        Logger.info('MapSwitcher', '设置WebSocket管理器');
        this.wsManager = wsManager;
        if (this.mapInstance) {
            Logger.debug('MapSwitcher', '注册GPS更新处理器');
            this.wsManager.onGPSUpdate((data) => {
                Logger.debug('MapSwitcher', '收到GPS更新:', data);
    ...
    handleGPSUpdate(data) {
        if (!this.mapInstance || !data) {
            Logger.warn('MapSwitcher', 'GPS更新处理失败:', {
                hasInstance: !!this.mapInstance,
                hasData: !!data
            });
            return;
        ...
        Logger.debug('MapSwitcher', '处理GPS更新:', data);
        
        // 检查是否只是状态更新（无坐标）
        if (!data.x || !data.y) {
            Logger.debug('MapSwitcher', '仅状态更新，无需添加新点位');
            // 只更新状态信息
            if (typeof this.mapInstance.updateGPSInfo === 'function') {
                this.mapInstance.updateGPSInfo(data);
            } else {
                Logger.warn('MapSwitcher', '地图实例不支持状态更新');
            }
            return;
        }
