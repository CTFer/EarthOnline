...
class Live2DService {
  ...
  async checkLibrariesLoaded() {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        try {
            if (typeof PIXI === "undefined" || !PIXI.live2d || !PIXI.live2d.Live2DModel) {
                Logger.warn("Live2DService", `第${i + 1}次检查: PIXI或Live2D库未加载`);
                continue;
            }
    ...
    // throw new Error('PIXI或Live2D库未加载');
  }
  ...
  async loadLive2DResources() {
    ...
    const live2dScripts = [
        "/static/js/live2d/pixi.min.js",           // PIXI必须最先加载
    ...
    try {
      for (const script of live2dScripts) {
        Logger.debug("Live2DService", `开始加载脚本: ${script}`);
