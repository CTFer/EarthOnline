...
class ExcelToSQLite:
    ...
    def _generate_create_table_sql(self):
        ...
        # 确定自增主键名称（如果用户数据中已有'id'列，则使用'primary_id'避免冲突）
        primary_key_name = 'id' if 'id' not in column_types else 'primary_id'
        
        # 生成建表SQL（IF NOT EXISTS 避免重复创建）
        create_sql = f"""
        CREATE TABLE IF NOT EXISTS `{self.table_name}` (
            {primary_key_name} INTEGER PRIMARY KEY AUTOINCREMENT,
            {columns_sql},
    ...
    def _process_data(self):
        ...
        for col in df_processed.columns:
            ...
            # 额外处理：将字符串形式的空值转换为真正的None
            df_processed[col] = df_processed[col].apply(
                lambda x: None if isinstance(x, str) and x.strip() in ['None', 'nan', 'NaN', ''] else x
            )
                ...
                try:
                    ...
                    if len(df_processed) > 0:
                        ...
                        for row in df_processed.values:
                            ...
                            processed_row = []
                            for val in row:
                                if val is None or (isinstance(val, str) and val.strip() in ['None', 'nan', 'NaN']):
